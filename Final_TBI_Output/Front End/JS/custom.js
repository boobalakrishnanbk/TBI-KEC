...
function convertToINRFormat(value, inputField) {
    ...
    if(number.toLocaleString('en-IN') == 'NaN'){
      $(inputField).val(0);
    }else{
...
function pickYear(value) {
  // $("#"+value).datepicker({dateFormat: 'yy'})
}
...
function addToClosingDisbursement(value){
  ...
  if((open - grant - oe - fab) >= 0){
    ...
    document.getElementById("id_closingBalance").value = (open - grant - oe - fab).toLocaleString('en-IN');
    $("#iderror").html("You have entered more amount. Please check your Entries Again!");
...
function addToClosingUtlization(value){
  ...
  if((open - grant - oe - fab + interest + returned) >= 0){
    document.getElementById("id_closingBalance").value = (open - grant - oe - fab + interest + returned).toLocaleString('en-IN');
    $("#submit").removeAttr("disabled");
    $("#iderror").html(" ");
  }
  else{
...
// edit
function editInstallment(id,x){
  var id = "ins-"+id.toString();
...
// edit
function editPatent(id,x){
  var id = "pat-"+id.toString();
  var tr = document.getElementById(id);
  var form = document.getElementById("addPatentForm");
  var dat = new Date(tr.cells[1].innerHTML)
